...
show_page = function (page_number) {
	...
	for (count = 0; count < page_list.length; count++) {
		if (count == page_number) page_list[count].slideDown();else page_list[count].hide();
	};
};
...
var Main = (function () {
	...
	var show_login_result = function (page_number) {
		...
		var count;
		for (count = 0; count < page_list.length; count++) {
			if (count == page_number) page_list[count].show();else page_list[count].hide();
		};
	};
	...
	var attachSendLogInInfo = function () {
		submit_log_in.on('click', function (e) {
			...
			if (uname == '' || pw == '') console.log('User did not input account and/or password');else {
				...
				var onSuccess = function (data) {
					if (data.status == 1) {
						...
						$('.log-in-form').slideToggle(); //Slide the form up
							...
							var i;
							for (i = 0; i < data.errors.length; i++) insertError(data.errors[i]);
							show_page(3);
							show_login_result(2);
						};
				};
	...
	var attachSendSignUpInfo = function () {
		submit_sign_up.on('click', function (e) {
			...
			if (uname == '' || pw == '' || rpw == '') console.log('User did not fill in all required areas');else {
				...
				var onFailure = function () {
					//send list of errors to error log
